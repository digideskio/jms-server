 ...
 * @constructor
 */
function ESfindDependencies (moduleData, done) {
	...
	try {
		...
		return done(new SyntaxError(['Line ', e.lineNumber, ': ', e.description, ' in file ', moduleData.path].join('')));
	}
...
DependencyMapper.prototype._transform = function (chunk, encoding, done) {
	...
	ESfindDependencies(data, function (err, result) {
