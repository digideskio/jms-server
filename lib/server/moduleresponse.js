 ...
 * @returns {Object}
 */
function moduleResponse (request, reply) {
...
//	console.log(request.moduleRequest);
	...
	if (moduleRequest.healthcheck) {
		return statusResponse(request, reply, moduleRequest);
	}
	...
	if (moduleRequest.invalid) {
		return responseHandler(request, reply, moduleRequest, new Exception415(moduleRequest.pathname));
	}
	...
	], responseHandler.bind(null, request, reply, moduleRequest));
