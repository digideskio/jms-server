...
function setupRequest (request, reply, moduleRequest) {
	...
	if (moduleRequest.prependClient) {
		moduleRequest.include.unshift('jms/client');
	}
...
function onMapResult (request, reply, moduleRequest, err, result) {
	...
	if (moduleRequest.prependClient && compactResult.length === 1) {
		// replace "jms/client" with actual module hash
		moduleRequest.include.splice(0, 1, compactResult[0])
		compactResult.shift();
	}
	...
	if (result.indexOf(null) > -1) {
		...
		if (moduleRequest.prependClient) {
			// no need to alter the url this much
			// (would insert a new module to the exclude field)
			moduleRequest.include.shift()
			result.shift()
		}
...
function ModuleRequest (server, options, next) {
	...
	server.ext('onPreHandler', function (request, reply) {
		...
		if (request.server.settings.app.debug.timer) {
			moduleRequest.timer = [];
		}

		if (request.params.modules === 'jmsclient.js') {
